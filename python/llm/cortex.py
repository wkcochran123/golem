...
model = ConceptNet(5120,300)

import requests

def get_mood():
    try:
        response = requests.get("http://192.168.68.79/mood", timeout=2)
...
def apply_row(data,mood):
    token = data["token"]
    vector = np.array(data["vector"], dtype=np.float32).reshape(1, -1) * mood
	neurons.append = vector
    model.predict(vector)[0].tofile("/User/williamcochran/Code/golem/python/logits.data")
